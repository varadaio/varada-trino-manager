...
def apply(con: Connection, json_path: Path = None, csv_path: Path = None):
    if json_path:
        apply_from_json(con=con, file_path=json_path)
    elif csv_path:
        apply_from_csv(con=con, file_path=csv_path)
    else:
        logger.exception('No valid CSV or JSON to read from')
        raise exceptions.Exit(code=1)


def apply_from_csv(con: Connection, file_path: Path):
    with VaradaRest(con=con) as varada_rest, open(file_path) as fd:
...
def apply_from_json(con: Connection, file_path: Path):
    with VaradaRest(con=con) as varada_rest, open(file_path) as fd:
        for rule in load(fd):
            varada_rest.set_warmup_rule(json_data=rule)
